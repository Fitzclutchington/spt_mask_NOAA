...
{
	auto p = std::string(inpath);
	auto slash = p.find_last_of("/");
	auto dot = p.find_last_of(".");

	if((slash == std::string::npos && dot != std::string::npos)
	|| (slash != std::string::npos && dot != std::string::npos && dot > slash)){
		p = p.substr(0, dot);
	}
	return p + suffix;
}
...
{
    acspo::matrix<uchar> land_mask_copy = land_mask.clone();
    auto land_mask_cv = acspo_to_opencv(land_mask_copy);

    acspo::matrix<uchar> border_mask(land_mask.size());
    auto border_mask_cv = acspo_to_opencv(border_mask);

	int x, y;
	int height = land_mask_cv.size[0];
	int width = land_mask_cv.size[1];

	Mat element = getStructuringElement( MORPH_RECT,
	                                   Size( kernel_size, kernel_size ) );
	dilate(land_mask_cv, border_mask_cv,element);
    #pragma omp parallel for private(x,y)
	for(y=0;y<height;y++){
		for(x=0;x<width;x++){
		  border_mask_cv.at<uchar>(y,x) -= land_mask_cv.at<uchar>(y,x);
		}
	}
...
{
    assert(clear.size() == landmask.size());
    int elem = landmask.elem();
...
void generate_cloud_histogram_3d(const acspo::matrix<float> &d1, const acspo::matrix<float> &d2, const acspo::matrix<float> &d3, const float *lows, const float *highs, const float *steps, const acspo::matrix<uchar> &mask, Mat1f &hist)
{
	...
	printf("initialized variables\n");
    #pragma omp parallel for collapse(2) private(x,y,m,n,s)
	for(y = 0; y < height; ++y){
		for(x = 0; x < width; ++x){
...
{	
    const unsigned int rows = src.rows(), cols = src.cols();
