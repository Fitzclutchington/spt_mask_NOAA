...
ACSPOFile::ACSPOFile(const char *path, int omode)
{
...
template <> void
ACSPOFile::_readvar<float>(const char *name, Mat1f &data)
{
    ::readvar(ncid, name, data);
	CHECKMAT(data, CV_32FC1);
}

template <> void
ACSPOFile::_readvar<uchar>(const char *name, Mat1b &data)
{
    ::readvar(ncid, name, data);
	CHECKMAT(data, CV_8UC1);
}
